defmodule Skyscraper.ElevatorTest do
  ...
  describe ".cancel_request/2" do
    ...
    @tag additionals: %{
           ...
           direction: :up,
           outer_request: true,
           queue: Queue.build() |> Queue.push({7, :up})
         }
    test "removes current destination and takes next from queue when it is equal to provided one by params and is outer request",
         %{elevator: elevator} do
      assert elevator.destination == {2, :up}
      {instructions, elevator} = elevator |> Elevator.cancel_request({2, :up})
      assert instructions == [:notify_new_destination]
      assert elevator.destination == {7, :up}
    end

    @tag additionals: %{destination: {7, :up}, step: :moving, direction: :up}
    test "ignores the command unless destination is outer request",
         %{elevator: elevator} do
      {instructions, new_elevator} = elevator |> Elevator.cancel_request({7, :up})
      assert instructions == []
      assert elevator == new_elevator
    end

    @tag additionals: %{destination: {7, :up}, step: :moving, direction: :up, outer_request: true}
    test "ignores the command if destination doesn't equal the current one",
         %{elevator: elevator} do
      {instructions, new_elevator} = elevator |> Elevator.cancel_request({6, :up})
      assert instructions == []
      assert elevator == new_elevator
    end
  end
